...
public interface TblCartMapper {
	 ...
	 * @return 件数
	 */
	@Select(value="SELECT count(user_id) FROM tbl_cart WHERE user_id = #{userId}")
	int findCountByUserId(int userId);
	
	/**
	 * ユーザーIDを条件にユーザーIDを更新する（仮ユーザーIDだった場合にのみ値が更新される）
	 * @param userId ユーザーID
	 ...
	 * @return 件数
	 */
	@Select(value="SELECT count(id) FROM tbl_cart WHERE user_id = #{userId} AND product_id = #{productId}")
